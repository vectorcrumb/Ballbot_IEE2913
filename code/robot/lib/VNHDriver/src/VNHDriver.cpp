...
void TorqueMotor::begin(uint8_t pwm_pin, uint8_t ina_pin, uint8_t inb_pin, uint8_t cs_pin, float kt) {
    this->motor = VNHDriver(pwm_pin, ina_pin, inb_pin);
    this->setTorque(0);
    this->csPin = cs_pin;
    this->Kt = kt;
    this->torque_measured = 0;
    this->controller = PID(TORQUE_PID_KP, TORQUE_PID_KI, TORQUE_PID_KD, TORQUE_PID_MIN_U, TORQUE_PID_MAX_U, PID_DELTA_TIME);
}
...
void TorqueMotor::updateMotor(float refresh_rate) {
    ...
    this->output = this->controller.calculate(this->torque_setpoint, this->torque_measured, refreshRate);
    this->motor.setSpeed(255 * this->output / (TORQUE_PID_MAX_U - TORQUE_PID_MIN_U));
}

void TorqueMotor::TorqueMotor() {
